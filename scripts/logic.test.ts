...
console.log('\n── 장소 검색: 지역 거르기 ──');
{
  const P = (lat: number, lng: number) => ({ id: 'x', name: 'x', address: '', coord: { lat, lng } });
  eq('국내 검색에 서울은 남김', inRegion('KR', P(37.57, 126.98)), true);
  eq('국내 검색에 도쿄는 뺌', inRegion('KR', P(35.71, 139.81)), false);
  eq('해외 검색에 도쿄는 남김', inRegion('GLOBAL', P(35.71, 139.81)), true);
  eq('해외 검색에 부산은 뺌', inRegion('GLOBAL', P(35.16, 129.16)), false);
  eq('좌표 없으면 남김', inRegion('GLOBAL', { id: 'x', name: 'x', address: '' }), true);
}
