...
- [ ] **`supabase/expenses.sql` 실행** (SQL Editor) — 가계부 표 + 예약 번호 칸.
      **새 PR 머지 전후 아무 때나** (실행 전에도 앱은 동작, 가계부만 "아직 쓸 수 없습니다")
- [ ] **새 PR 머지** — 가계부·정산, 오늘 카드, 동선 최적화, 예약 번호·캘린더 내보내기
...
### 가계부 · 오늘 카드 · 동선 최적화 · 예약 번호 — 2026-09-24 (PR 대기)
...
- `supabase/expenses.sql`을 실제 DB에 돌려 본 적 없음 (문법은 기존 SQL 파일 모양을 따름)
- 캘린더 파일을 구글·애플 캘린더에 실제로 넣어 본 적 없음 (형식은 표준대로, 테스트로 확인)

## 💡 다음 후보
- 후보 장소 투표 + 일정 댓글 (벤치마크 3순위 — Wanderlog의 대표 불만을 메우는 차별점)
