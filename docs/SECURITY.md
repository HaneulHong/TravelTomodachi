...
- **개발 도구 취약점**: Vite 5(높음 — 개발 서버 경로 탐색)·esbuild(중간). **개발 서버를 켰을 때만**
  해당된다. `vite.config.ts`가 `host: true`라 같은 와이파이에서 개발 서버에 접근할 수 있으니, 카페 같은
  공용 와이파이에서는 개발 서버를 켜 두지 않는다. Vite 8로 올리면 사라진다(큰 업그레이드 — 따로).
