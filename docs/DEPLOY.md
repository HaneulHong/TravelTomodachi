...
3. GitHub 연결 → `HaneulHong/TravelTomodachi` 선택
4. 빌드 설정
   ...
   Node 버전은 저장소의 `.node-version`(22)을 자동으로 따른다.

5. **Environment variables** (Production) — `.env.local`과 같은 이름·값
   ...
   | `VITE_PUBLIC_BASE_URL` | 배포 주소 (예: `https://traveltomodachi.pages.dev`) |

   ⚠️ **secret / service_role 키는 절대 넣지 않는다.** 여기 값은 전부 번들에
   들어가 누구나 볼 수 있다. 공개 키만 넣는다(접근 제어는 RLS가 한다).

   `VITE_PUBLIC_BASE_URL`은 프로젝트 이름을 정해야 안다. 처음엔 비워 두고
   배포한 뒤 주소를 확인해 넣고 **다시 배포**(Deployments → Retry)한다.
   이 값이 초대 링크의 주소가 된다.

6. **Save and Deploy** → 몇 분 뒤 `https://<프로젝트이름>.pages.dev`
...
배포 주소를 `https://traveltomodachi.pages.dev`라고 하면:
...
처음엔 무료 `*.pages.dev` 주소로 충분하다.

내 도메인을 쓰려면 사서(.com 등은 Cloudflare Registrar에서 원가, .kr은 국내
업체) Pages 프로젝트 → **Custom domains**에 연결한다.
...
| 서비스 | 한도 | 넘으면 |
|---|---|---|
| Cloudflare Pages | 빌드 월 500회, 트래픽 무제한 | — |
